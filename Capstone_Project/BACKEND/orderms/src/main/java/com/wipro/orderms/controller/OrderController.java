...
public class OrderController {
  ...
  // 1) Create order: /order (POST)
  @PostMapping
  public Order create(@Valid @RequestBody OrderCreateRequest req) { return svc.create(req); }

  // 2) Cancel order: /order/{orderId}?userId=... (PUT)
  @PutMapping("/{orderId}")
  public Order cancel(@PathVariable Integer orderId, @RequestParam Integer userId) {
    return svc.cancel(orderId, userId);
  }
